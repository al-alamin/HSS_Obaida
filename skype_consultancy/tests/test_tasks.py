...
from skype_consultancy.tasks import send_mail_async, send_remainder_email, skype_event_group_email
...
class SendRemainderEmailTest(TestCase):
    ...
    def setUp(self):
        ...
        self.user2 = User.objects.create_user("alamin2", "a2@gmail.com")
    ...
    def test_send_remainder_email(self):
        response = send_remainder_email.delay(self.event.id)
...
class SkypeEventGroupEmailTest(TestCase):
    ...
    def setUp(self):
        ...
        Registration.objects.create(event=self.event,
                                    attendee=self.user2, skype_id="skype_id")
        self.event_email = EventEmail.objects.create(event=self.event, email_subject="Hi", email_body="body")
